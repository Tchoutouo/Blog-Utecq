    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <form action=" {{route('article.update', $article->id)}}" method="POST" enctype="multipart/form-data" class="ml-3 mr-3">
                    @csrf
                    @method('PUT')
                    <div class="form-group mt-3">
                        <label class="fw-bolder">Titre<span class="text-danger">*</span></label>
                        <input type="text" name="titre" class="form-control rounded-2" value="{{ $article->titre }}">
                    </div>
                    <div class="form-group mt-3">
                        <label class="fw-bolder">Contenu<span class="text-danger">*</span></label>
                        <textarea name="contenu" id="" cols="30" rows="10" style="width: 100%" class="rounded-2">{{ $article->contenu }}</textarea>
                    </div>
                    <div class="row mt-3">
                        <div class="col-md-6">
                            <div class="form-group">
                                <label class="fw-bolder">Auteur</label>
                                <input type="text" name="auteur" class="form-control rounded-2" value="{{$user_online->name}}" disabled>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                    ...
                    <input type="text" name="user_id" class="form-control rounded-2" value="{{$user_online->id}}" placeholder="" hidden>
